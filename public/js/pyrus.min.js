        ...
        dataAUX.forEach( function( x ) {
            ...
            this.columnas().forEach( function( column ) {
                ...
                if( this.especificacion[ column.COLUMN ].TIPO == "TP_IMAGE" ) {
                    ...
                    element = `<img class="w-100" src="${img}" onerror="this.src='${src}'"/>`;
                    if( info !== undefined ) {
                        element += `<p class="text-center mt-2"><strong class="mr-1">Dimensiones:</strong>${info[ 0 ]}px x ${info[ 1 ]}px</p>`;
                        element += `<p class="text-center mt-1"><strong class="mr-1">Tipo:</strong>${info.mime}</p>`;
                    }
                }
            ...
            buttons = `<td class="text-center">`;
                if( buttonsOK.indexOf( "c" ) >= 0 )
                    buttons += `<button style="width: 35px; font-size: 13px;" onclick="clone(this,'${id}')" class="btn text-center btn-info rounded-0" disabled><i class="far fa-clone"></i></button>`;
                if( buttonsOK.indexOf( "e" ) >= 0 )
                    buttons += `<button style="width: 35px; font-size: 13px;" onclick="edit(this,'${id}')" class="btn text-center rounded-0 btn-warning"><i class="fas fa-pencil-alt"></i></button>`;
                if( buttonsOK.indexOf( "d" ) >= 0 )
                    buttons += `<button style="width: 35px; font-size: 13px;" onclick="erase(this,'${id}')" class="btn text-center rounded-0 btn-danger"><i class="fas fa-trash-alt"></i></button>`;
                if( button !== null ) {
                    button.forEach( function( b ) {
                        buttons += `<button style="width: 35px; font-size: 13px;" onclick="${b.function}Function(this,'${id}')" class="btn text-center rounded-0 ${b.class}">${b.icon}</button>`;
                    });
                }
            buttons += `</td>`;
            row.push( buttons );
                ...
                for(let i in rowElements) {
                    ...
                    rowElementos.forEach( function( e ) {
                        ...
                        if(e == "VACIO") {
                            ...
                            aux = this.inputAdecuado(this.especificacion[e], e, id, STR_class, OBJ_funcion, this.especificacion[e].PLACEHOLDER === undefined ? "" : this.especificacion[e].PLACEHOLDER, multiple , e );
                            ...
                            if((this.especificacion[e].TIPO == "TP_FECHA" && this.especificacion[e].SIMPLE === undefined) || (this.especificacion[e].MULTIPLE !== undefined && this.especificacion[e].MULTIPLE))
                                aux = `<label class="" for="${AUX_nombre}">${this.especificacion[e].NOMBRE}</label>` + aux;
                            ...
                            if( this.objeto.IDIOMAS !== undefined ) {
                                if( this.objeto.IDIOMAS[e] !== undefined ) {
                                    ...
                                    for( let j in this.objeto.IDIOMAS[ e ].ELEMENT ) {
                                        placeholder = this.especificacion[e].NOMBRE === undefined ? e : this.especificacion[e].NOMBRE;
                                        placeholder = placeholder.toUpperCase();
                                        placeholder += ` ${this.objeto.IDIOMAS[ e ].ELEMENT[ j ]}`;
                                        
                                        auxR = this.objeto.IDIOMAS[ e ].FORM;
                                        auxX = this.inputAdecuado(this.especificacion[e], `${e}_${j}`, id, STR_class, OBJ_funcion, placeholder , multiple , e );
                                        if(this.objeto.PLACEHOLDER !== undefined)
                                            auxX += `<label for="">${this.especificacion[e].PLACEHOLDER === undefined ? this.especificacion[e].NOMBRE : this.especificacion[e].PLACEHOLDER}</label>`;
                                        if(this.especificacion[e].AYUDA !== undefined)
                                            auxX += `<small class="form-text text-muted">${this.especificacion[e].AYUDA}</small>`;
        ...
        if(STR_type == "file") {
            ...
            if( Object_.TIPO == "TP_IMAGE" || Object_.TIPO == "TP_BLOB" ) {
                ...
                if(Object_.WIDTH !== undefined)
                    w = Object_.WIDTH;
                input = `<img id="src-${Arr.idElementForm}" class="w-100 mx-auto d-block mb-1 img-thumbnail bg-dark" style="max-width: ${w};" src="" onError="this.src='${urlFile}'" /><div class="custom-file">${input}</div>`;
            } else
