...
<script>
    ...
    window.modify = null;
    window.categorias = @json( $data[ "categorias" ] );
    window.pyrus = new Pyrus( "productos" , { categoria_id: { DATA : window.categorias } } , src );
    ...
    add = function(t, id = 0, data = null) {
        ...
        if( window.categoria_id != "" ) {
            $( `#${window.pyrus.name}_categoria_id` ).val( window.categoria.id );
        }
    ...
    imagesFunction = function( t , id ) {
        ...
        let producto = window.elementos.data[ index ];
    ...
    init = function( callbackOK ) {
        ...
        window.pyrus.elements( $( "#tabla" ) , `{{ asset('/') }}` , window.elementos.data , [ "e" , "d" ] , [ { icon : '<i class="fas fa-images"></i>' , class: 'btn-primary' , function : 'images' } ] );
